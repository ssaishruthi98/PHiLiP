        ...
        std::shared_ptr<DGBase<dim,nspecies,real,MeshType>> dg) const
{
    ...
    const unsigned int nstate = dim + nspecies + 1;
    ...
    std::shared_ptr < Physics::Euler<dim, nspecies, dim+nspecies+1, double > > euler_physics = std::dynamic_pointer_cast<Physics::Euler<dim,nspecies,dim+nspecies+1,double>>(
                Physics::PhysicsFactory<dim,nspecies,dim+nspecies+1,double>::create_Physics(&parameters_using_euler_pde_type));
