...
void AllParameters::declare_parameters (dealii::ParameterHandler &prm)
{
    ...
    prm.declare_entry("flux_reconstruction_aux", "kDG",
                      dealii::Patterns::Selection(
                      "kDG | kSD | kHU | kNegative | kNegative2 | kPlus | k10Thousand | kAdaptive"),
                      "Flux Reconstruction for Auxiliary Equation. "
                      "Choices are <kDG | kSD | kHU | kNegative | kNegative2 | kPlus | k10Thousand | kAdaptive>.");
...
const std::string test_string = prm.get("test_type");
    ...
    if (flux_reconstruction_aux_string == "kAdaptive") { flux_reconstruction_aux_type = kAdaptive; }
