#include "h_refinement_study_sod_shock_tube.h"
#include "flow_solver/flow_solver_factory.h"
#include "flow_solver/flow_solver_cases/positivity_preserving_tests.h"
...
HRefinementStudySodShockTube<dim, nstate>::HRefinementStudySodShockTube(
...
template <int dim, int nstate>
Parameters::AllParameters HRefinementStudySodShockTube<dim,nstate>::reinit_params_and_refine(int refinement) const
{
     PHiLiP::Parameters::AllParameters parameters = *(this->all_parameters);
     
     parameters.flow_solver_param.number_of_grid_elements_per_dimension *= pow(2, refinement);
     parameters.flow_solver_param.number_of_grid_elements_x = parameters.flow_solver_param.number_of_grid_elements_per_dimension;
...
template <int dim, int nstate>
void HRefinementStudySodShockTube<dim,nstate>::calculate_Lp_error_at_final_time_wrt_function(double &Lp_error_density, 
        ...
        int norm_p) const
{
    ...
    for (auto cell : dg->dof_handler.active_cell_iterators()) {
        ...
        for (unsigned int iquad=0; iquad<n_quad_pts; ++iquad) {
            ...
            bool shock_location = false;
            // if (qpoint[0] > 0.6854905240097902 - 1e-1 && qpoint[0] < 0.6854905240097902 + 1e-1) {
            //     shock_location = true;
            // }

            // if (qpoint[0] > 0.8504311464060357 - 1e-1 && qpoint[0] < 0.8504311464060357 + 1e-1) {
            //     shock_location = true;
            // }
...
template <int dim, int nstate>
int HRefinementStudySodShockTube<dim, nstate>::run_test() const
{
    ...
    for (int refinement = 4; refinement < n_calculations; ++refinement){
        ...
        std::unique_ptr<FlowSolver::PositivityPreservingTests<dim, nstate>> flow_solver_case = std::make_unique<FlowSolver::PositivityPreservingTests<dim,nstate>>(&params);
        ...
        const double dt = flow_solver_case->get_adaptive_time_step(flow_solver->dg);
...
#if PHILIP_DIM==1
    template class HRefinementStudySodShockTube<PHILIP_DIM,PHILIP_DIM+2>;
#endif
} // Tests namespace
} // PHiLiP namespace
