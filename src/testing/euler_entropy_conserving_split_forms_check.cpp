...
int EulerSplitEntropyCheck<dim, nspecies, nstate>::run_test() const
{
    ...
    for (unsigned int i = 0; i < n_fluxes; ++i){
        ...
        if (i==0 && this->all_parameters->pde_type == PDE_enum::multispecies_thermally_perfect_euler) {
            pcout << "-----------------------------------------------------------------------" << std::endl;
            pcout << "   Skipping " << flux_names[i] << " two-point flux. MS TPG does not have this flux implemented." << std::endl;
            pcout << "-----------------------------------------------------------------------" << std::endl;
        } else {
