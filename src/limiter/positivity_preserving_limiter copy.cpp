...
std::array<real, nstate> PositivityPreservingLimiter<dim, nstate, real>::get_soln_cell_avg_2D(
    ...
    const unsigned int                                                      n_quad_pts,
    const std::array<std::vector<real>, dim>&                               quad_weights,
    double                                                                  dt)
{
    ...
    double                                                  dt)
{
    ...
    std::array<std::vector<real>,dim>& quad_weights;

    if(dim == 1) {
        quad_weights[0] = oneD_quad_GLL.weights();
    } else if(dim == 2) {
        dealii::Quadrature<2> quad_xGLL(oneD_quad_GLL,oneD_quad_GL);
        dealii::Quadrature<2> quad_yGLL(oneD_quad_GL,oneD_quad_GLL);

        quad_weights[0] = quad_xGLL.weights();
        quad_weights[1] = quad_yGLL.weights();
    } else if(dim == 2) {
        dealii::QGauss<2> twoD_quad_GL(max_degree + 1);
        dealii::Quadrature<3> quad_xGLL(oneD_quad_GLL,oneD_quad_GL);
        dealii::Quadrature<3> quad_yGLL(oneD_quad_GL,oneD_quad_GLL);
    ...
    for (auto soln_cell : dof_handler.active_cell_iterators()) {
        ...
        const unsigned int n_quad_pts = quad_xGLL.size();
        
        std::array<std::array<std::vector<real>, nstate>, dim> soln_at_q;
        std::array<std::vector<real>, nstate> soln_at_q_oneD;

        // Interpolate solution dofs to quadrature pts.
        for(int idim = 0; idim < dim; idim++) {
            for (int istate = 0; istate < nstate; istate++) {
                soln_at_q_oneD[istate].resize(n_quad_pts); 
                if(idim == 1)
                    soln_basis_GLL.matrix_vector_mult(soln_coeff[istate], soln_at_q_oneD[istate],
                        soln_basis_GLL.oneD_vol_operator, soln_basis_GL.oneD_vol_operator, soln_basis_GLL.oneD_vol_operator);
                else if(idim == 2)
                    soln_basis_GLL.matrix_vector_mult(soln_coeff[istate], soln_at_q_oneD[istate],
                        soln_basis_GL.oneD_vol_operator, soln_basis_GLL.oneD_vol_operator, soln_basis_GLL.oneD_vol_operator);
                else if(idim == 3)
                    soln_basis_GLL.matrix_vector_mult(soln_coeff[istate], soln_at_q_oneD[istate],
                        soln_basis_GLL.oneD_vol_operator, soln_basis_GLL.oneD_vol_operator, soln_basis_GL.oneD_vol_operator);
            }
            soln_at_q[idim] = soln_at_q_oneD;
        }
        ...
        // Obtain solution cell average
        std::array<real, nstate> soln_cell_avg = get_soln_cell_avg_2D(soln_at_q, n_quad_pts, quad_weights, dt);
