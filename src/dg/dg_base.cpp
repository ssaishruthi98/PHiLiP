...
{   
    ...
    OPERATOR::local_Flux_Reconstruction_operator<dim,2*dim,real> reference_FR(1, max_degree, init_grid_degree, FR_Type, FR_user_specified_correction_parameter_value);
        ...
        const bool use_auxiliary_eq)
{
    ...
    OPERATOR::FR_mass_inv<dim,2*dim,real> mass_inv(1, max_degree, init_grid_degree, FR_Type, FR_user_specified_correction_parameter_value);
    ...
    OPERATOR::FR_mass_inv<dim, 2 * dim, real> mass_inv_cHU(1, max_degree, init_grid_degree, FR_enum::cHU, true);
    ...
    double c_HU_value = mass_inv_cHU.FR_param;
    // ************************* Adaptive Flux Reconstruction Steps ************************* //
    ...
    if(Cartesian_first_element){//then we can factor out det of Jac and rapidly simplify
        if(use_auxiliary_eq){
            ...
            mass_inv_cHU.build_1D_volume_operator(oneD_fe_collection_1state[max_degree], oneD_quadrature_collection[max_degree]);
            ...
            c_HU_value = mass_inv_cHU.FR_param;
            // ************************* Adaptive Flux Reconstruction Steps ************************* //
        }
    }
    ...
    for (auto soln_cell = dof_handler.begin_active(); soln_cell != dof_handler.end(); ++soln_cell, ++metric_cell, ++cell_index) {
        ...
        if(FR_Type == FR_enum::cAdaptive && sensor_type == Sensor_enum::jameson_sensor){
            sensor_value = jameson_sensor(output_vector, fe_collection[poly_degree], current_dofs_indices);
            this->jameson_sensor_cell[cell_index] = sensor_value;
        } 
        else if(FR_Type == FR_enum::cAdaptive && sensor_type == Sensor_enum::modal_sensor){
            sensor_value = modal_sensor(volume_quadrature_collection[poly_degree], soln_dofs, fe_collection[poly_degree],metric_oper.det_Jac_vol);
            this->modal_sensor_cell[cell_index] = sensor_value;
        }
        ...
        const bool use_unmodified_mass_matrix)
{
    ...
    OPERATOR::FR_mass<dim,2*dim,real> mass(1, max_degree, init_grid_degree, FR_Type, FR_user_specified_correction_parameter_value);
