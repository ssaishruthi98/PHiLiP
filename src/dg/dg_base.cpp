...
void DGBase<dim,real,MeshType>::output_results_vtk (const unsigned int cycle, const double current_time)// const
{
    ...
    data_out.add_data_vector(jameson_sensor_cell, std::string("jameson_sensor"), dealii::DataOut_DoFData<dealii::DoFHandler<dim>,dim>::DataVectorType::type_cell_data);
    ...
    data_out.add_data_vector(c_value_cell, std::string("c_value"), dealii::DataOut_DoFData<dealii::DoFHandler<dim>,dim>::DataVectorType::type_cell_data);
    // ************************* Adaptive Flux Reconstruction Steps ************************* //
        ...
        const bool use_auxiliary_eq)
{
    ...
    OPERATOR::FR_mass_inv_aux<dim, 2 * dim, real> mass_inv_kDG(1, max_degree, init_grid_degree, FR_Aux_enum::kDG);
    OPERATOR::FR_mass_inv_aux<dim, 2 * dim, real> mass_inv_kPlus(1, max_degree, init_grid_degree, FR_Aux_enum::kPlus);
    OPERATOR::FR_mass_inv_aux<dim, 2 * dim, real> mass_inv_k10Thousand(1, max_degree, init_grid_degree, FR_Aux_enum::k10Thousand);
    // ************************* Adaptive Flux Reconstruction Steps ************************* //
    ...
    mass_inv_kDG.build_1D_volume_operator(oneD_fe_collection_1state[max_degree], oneD_quadrature_collection[max_degree]);
    mass_inv_kPlus.build_1D_volume_operator(oneD_fe_collection_1state[max_degree], oneD_quadrature_collection[max_degree]);
    mass_inv_k10Thousand.build_1D_volume_operator(oneD_fe_collection_1state[max_degree], oneD_quadrature_collection[max_degree]);
    // ************************* Adaptive Flux Reconstruction Steps ************************* //
    ...
    for (auto soln_cell = dof_handler.begin_active(); soln_cell != dof_handler.end(); ++soln_cell, ++metric_cell, ++cell_index) {
        ...
        for(int istate=0; istate<nstate; istate++){
            ...
            if(Cartesian_element){
                if(use_auxiliary_eq){
                    // ************************* Adaptive Flux Reconstruction Steps ************************* //
                    if (FR_Type_Aux == FR_Aux_enum::kAdaptive) {
                        if (modal_sensor_value > this->all_parameters->shock_sensor_threshold) {
                            mass_inv_kPlus.matrix_vector_mult_1D(local_input_vector, local_output_vector,
                                mass_inv_kPlus.oneD_vol_operator,
                                false, 1.0 / metric_oper.det_Jac_vol[0]);
                            this->c_value_cell[cell_index] = 1.0;
                        }
                        
                        else {
                            mass_inv_kDG.matrix_vector_mult_1D(
                                local_input_vector, local_output_vector,
                                mass_inv_kDG.oneD_vol_operator,
                                false, 1.0 / metric_oper.det_Jac_vol[0]);
