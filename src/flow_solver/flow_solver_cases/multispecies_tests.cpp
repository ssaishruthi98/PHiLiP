...
void MultispeciesTests<dim, nspecies, nstate>::compute_and_update_integrated_quantities(DGBase<dim, nspecies, double> &dg)
{
    ...
    for (auto cell = dg.dof_handler.begin_active(); cell!= dg.dof_handler.end(); ++cell, ++metric_cell) {
        ...
        for (unsigned int iquad=0; iquad<n_quad_pts; ++iquad) {
            ...
            for(int istate=0; istate<nstate; istate++){
                ...
                if (istate == 0 && (soln_state[istate] < 0 || soln_state[istate] != soln_state[istate])) {
                    std::cout << "Flow Solver Error: Total density is a nonphysical value - Aborting... " << std::endl << std::flush;
                    std::abort();
                }
                if (istate == dim+1 && (soln_state[istate] < 0 || soln_state[istate] != soln_state[istate])) {
            ...
            for(int istate=0; istate<nstate; istate++){
                ...
                soln_basis_projection_oper.matrix_vector_mult_1D(energy_var_vol_int[istate], energy_var_hat[istate],
            ...
            for (unsigned int flux_basis_quad=iquad; flux_basis_quad<n_quad_pts; ++flux_basis_quad) {
                ...
                for(int idim=0; idim<dim; idim++){
                    for(int jdim=0; jdim<dim; jdim++){
                        metric_cofactor_flux_basis[idim][jdim] = metric_oper.metric_cofactor_vol[idim][jdim][flux_basis_quad];
                    }
                }
                ...
                for(int istate=0; istate<nstate; istate++){
                    ...
                    metric_oper.transform_physical_to_reference(
                        ...
                        conv_ref_flux_2pt[flux_basis_quad][istate]);
                }
            }
            ...
            for(int istate=0; istate<nstate; istate++){
                ...
                for(int idim=0; idim<dim; idim++){
                    ...
                    for (unsigned int flux_basis_quad=iquad; flux_basis_quad<n_quad_pts; ++flux_basis_quad) {
                        ...
                        conv_ref_2pt_flux_at_q[istate][idim][iquad][flux_basis_quad] = conv_ref_flux_2pt[flux_basis_quad][istate][idim];
                        conv_ref_2pt_flux_at_q[istate][idim][flux_basis_quad][iquad] = conv_ref_flux_2pt[flux_basis_quad][istate][idim];
                    }
                }
            }
        ...
        integral_values[IntegratedQuantitiesEnum::volume_term] = volume_term;
    }
    ...
    for(int i_quantity=0; i_quantity<NUMBER_OF_INTEGRATED_QUANTITIES; ++i_quantity) {
        ...
        if (i_quantity == IntegratedQuantitiesEnum::kinetic_energy) {
...
template<int dim, int nspecies, int nstate>
double MultispeciesTests<dim, nspecies, nstate>::compute_volume_term(const std::shared_ptr < DGBase<dim, nspecies, double> > &dg, unsigned int poly_degree) const
{
...
template<int dim, int nspecies, int nstate>
double MultispeciesTests<dim, nspecies, nstate>::get_volume_term() const
{
    const double integrated_volume_term = this->integrated_quantities[IntegratedQuantitiesEnum::volume_term];
    ...
    const bool do_write_unsteady_data_table_file)
{
    ...
    double current_vol_work = compute_volume_term(dg, dg->max_degree);
    std::cout << " current vol work ??? " << current_vol_work << std::endl;
    ...
    double current_volume_term = 0.0;
    current_volume_term = this->get_volume_term(); // no overintegration
    if (current_iteration == 0)  initial_volume_term = current_volume_term;
    ...
    if (this->mpi_rank == 0) {
        ...
        this->add_value_to_data_table(current_volume_term,"current_volume_term",unsteady_data_table);
        unsteady_data_table->set_scientific("current_volume_term", false);
        this->add_value_to_data_table(current_volume_term-initial_volume_term,"change_in_volume_term",unsteady_data_table);
        unsteady_data_table->set_scientific("change_in_volume_term", false);
        // Write to file
    ...
    if (current_iteration % this->all_param.ode_solver_param.print_iteration_modulo == 0) {
        ...
        this->pcout << "    Current Kinetic Energy:  " << current_kinetic_energy
                    << "    KE-KE_0:  " << current_kinetic_energy-initial_kinetic_energy;
        this->pcout << "    Current Volume Term:  " << current_kinetic_energy
                    << "    Change in Volume Term:  " << current_kinetic_energy-initial_kinetic_energy;
        this->pcout << std::endl;
    }
