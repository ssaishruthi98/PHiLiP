...
{ 
    this->ms_physics = std::dynamic_pointer_cast<Physics::MultiSpecies_CaloricallyPerfect_Euler<dim,nspecies,dim+nspecies+1,double>>(
...
template<int dim, int nspecies, int nstate>
double MultispeciesTests<dim, nspecies, nstate>::compute_volume_term(std::shared_ptr < DGBase<dim, nspecies, double> > dg) const
{
    ...
    for (auto cell = dg->dof_handler.begin_active(); cell!= dg->dof_handler.end(); ++cell, ++metric_cell) {
        ...
        for(unsigned int iquad=0; iquad<n_quad_pts; iquad++){
            ...
            energy_var = this->ms_physics->compute_kinetic_energy_variables(soln_state);
            for(int istate=0; istate<nstate; istate++){
                if(iquad==0){
        ...
        for (unsigned int iquad=0; iquad<n_quad_pts; ++iquad) {
            ...
            for (unsigned int flux_basis=iquad; flux_basis<n_quad_pts; ++flux_basis) {
                ...
                conv_phys_flux_2pt = this->ms_physics->convective_numerical_split_flux(soln_state, soln_state_flux_basis);

                //Need to subtract off the pressure average term
                const double pressure_int = this->ms_physics->compute_pressure(soln_state);
                const double pressure_ext = this->ms_physics->compute_pressure(soln_state_flux_basis);
                for(int idim=0; idim<dim; idim++){
    ...
    volume_term = dealii::Utilities::MPI::sum(volume_term, this->mpi_communicator);
    return volume_term;
}
...
double MultispeciesTests<dim, nspecies, nstate>::compute_integrated_entropy(DGBase<dim, nspecies, double> &dg) const
{
    ...
    for (auto cell = dg.dof_handler.begin_active(); cell!= dg.dof_handler.end(); ++cell, ++metric_cell) {
        ...
        for (unsigned int iquad=0; iquad<n_quad_pts; ++iquad) {
            ...
            //#####################################################################
            const double quadrature_entropy = this->ms_physics->compute_numerical_entropy_function(soln_at_q);
    ...
    const bool do_write_unsteady_data_table_file)
{
    ...
    if (current_iteration == 0)  initial_volume_work = current_volume_work;
    ...
    if (this->mpi_rank == 0) {
        ...
        this->add_value_to_data_table(current_volume_work,"current_volume_work",unsteady_data_table);
        unsteady_data_table->set_scientific("current_volume_work", false);
        this->add_value_to_data_table(current_volume_work-initial_volume_work,"KE-KE_0",unsteady_data_table);
    ...
    if (current_iteration % this->all_param.ode_solver_param.print_iteration_modulo == 0) {
        ...
        this->pcout << "    Current Volume Work:  " << current_volume_work
                    << "    KE-KE_0:  " << current_volume_work-initial_volume_work;
        this->pcout << std::endl;
    }
