...
// Ismail-Roe entropy-conserving two-point flux.
template <int dim, int nspecies, int nstate, typename real>
std::array<dealii::Tensor<1,dim,real>,nstate> Multispecies_CaloricallyPerfect_Euler<dim, nspecies, nstate, real>
                                                 ...
                                                 const std::array<real,nstate> &conservative_soln2) const
{
    ...
    // compute the sqrt of TL, TR, 1/TL, 1/TR and the corresponding avgs and log means
    ...
    real sum_of_Rk_rhok = 0.0; // numerator for pressure diagonal term
    real mean_density = 0.0;
    for (int ispecies = 0; ispecies < nspecies; ispecies++){
...
// Chandrashekar entropy-conserving two-point flux.
template <int dim, int nspecies, int nstate, typename real>
std::array<dealii::Tensor<1,dim,real>,nstate> Multispecies_CaloricallyPerfect_Euler<dim, nspecies, nstate, real>
...
// Ranocha entropy-conserving two-point flux.
template <int dim, int nspecies, int nstate, typename real>
std::array<dealii::Tensor<1,dim,real>,nstate> Multispecies_CaloricallyPerfect_Euler<dim, nspecies, nstate, real>
                                  ...
                                  const std::array<real,nstate> &conservative_soln2) const
{
    ...
    if(this->two_point_num_flux_type == two_point_num_flux_enum::KG) {
        ...
        std::cout << "The Chandrashekar two-point flux has not been implemented for multispecies thermally perfect gas...Aborting." << std::endl;
        std::abort();
    } else if(this->two_point_num_flux_type == two_point_num_flux_enum::Ra) {
        std::cout << "The Ranocha fix for the Chandrashekar two-point flux has not been implemented for multispecies thermally perfect gas...Aborting." << std::endl;
        std::abort();
    }
