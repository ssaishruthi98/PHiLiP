        ...
        const int boundary_type) const
{
    ...
    if((on_boundary && (thermal_boundary_condition_type == thermal_boundary_condition_enum::adiabatic))
        ...
        // Step 1: Primitive solution
        const std::array<real,nstate> primitive_soln = this->template convert_conservative_to_primitive<real>(solution); // from Euler

        // Step 2: Gradient of primitive solution
        const std::array<dealii::Tensor<1,dim,real>,nstate> primitive_soln_gradient = this->template convert_conservative_gradient_to_primitive_gradient<real>(solution, solution_gradient);
