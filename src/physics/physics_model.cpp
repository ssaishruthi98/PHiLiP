        ...
        const int boundary_type) const
{
    ...
    std::array<real,nstate_baseline_physics> baseline_diss_flux_dot_n
        = physics_baseline->dissipative_flux_dot_normal(baseline_conservative_soln, baseline_solution_gradient, on_boundary, cell_index, normal, boundary_type);

    // Initialize diss_flux_dot_n as the model dissipative flux dot normal
    std::array<real,nstate> diss_flux_dot_n = model->dissipative_flux_dot_normal(solution, solution_gradient, on_boundary, cell_index, normal, boundary_type);
