...
// ========================================================
template <int dim, int nspecies, int nstate, typename real>
InitialConditionFunction_MultiSpecies_CaloricallyPerfectBase<dim, nspecies, nstate, real>
::InitialConditionFunction_MultiSpecies_CaloricallyPerfectBase(
    Parameters::AllParameters const* const param)
    : InitialConditionFunction<dim, nspecies, nstate, real>()
{
    // Real Gas object; create using dynamic_pointer_cast and the create_Physics factory
    PHiLiP::Parameters::AllParameters parameters_multispecies_calorically_perfect = *param;
    parameters_multispecies_calorically_perfect.pde_type = Parameters::AllParameters::PartialDifferentialEquation::multispecies_calorically_perfect;
    this->multispecies_calorically_perfect_physics = std::dynamic_pointer_cast<Physics::MultiSpecies_CaloricallyPerfect<dim,nspecies,dim+nspecies+1,double>>(
                Physics::PhysicsFactory<dim,nspecies,dim+nspecies+1,double>::create_Physics(&parameters_multispecies_calorically_perfect));
}

template <int dim, int nspecies, int nstate, typename real>
real InitialConditionFunction_MultiSpecies_CaloricallyPerfectBase<dim, nspecies, nstate, real>
::convert_primitive_to_conversative_value(
    const dealii::Point<dim, real>& point, const unsigned int istate) const
{
    real value = 0.0;
    ...
    const std::array<real, nstate> soln_conservative = this->multispecies_calorically_perfect_physics->convert_primitive_to_conservative(soln_primitive);
    value = soln_conservative[istate];
...
template <int dim, int nspecies, int nstate, typename real>
inline real InitialConditionFunction_MultiSpecies_CaloricallyPerfectBase<dim, nspecies, nstate, real>
::value(const dealii::Point<dim, real>& point, const unsigned int istate) const
{
    real value = 0.0;
    value = convert_primitive_to_conversative_value(point, istate);
...
::InitialConditionFunction_Multispecies_VortexAdvection(
    ...
    : InitialConditionFunction_MultiSpecies_CaloricallyPerfectBase<dim,nspecies,nstate,real>(param)
...
::primitive_value(const dealii::Point<dim,real> &point, const unsigned int istate) const
{
    ...
    const std::array<real,nspecies> Rs = this->multispecies_calorically_perfect_physics->compute_Rs();
    real y_O2;
    real R_mixture;
    // For a 2 species test
    if constexpr(nspecies==2 && nstate==dim+nspecies+1) {
        y_O2 = 1.0 - y_H2;
        R_mixture = (y_H2*Rs[0] + y_O2*Rs[1])*this->multispecies_calorically_perfect_physics->R_ref;
    }
    ...
    if constexpr(nspecies==3 && nstate==dim+nspecies+1) {
        ...
        R_mixture = (y_H2*Rs[0] + y_O2*Rs[1] + y_N2*Rs[2])*this->multispecies_calorically_perfect_physics->R_ref;
    }
    ...
    if(istate==0) {
        // mixture density
        value = density / this->multispecies_calorically_perfect_physics->density_ref;
    }
    if(istate==1) {
        // x-velocity
        value = velocity / this->multispecies_calorically_perfect_physics->u_ref;
    }
    if(dim==2 && istate==2) {
        // y-velocity
        value = velocity / this->multispecies_calorically_perfect_physics->u_ref;
    ...
    if(istate==dim+1) {
        // pressure
        value = pressure / (this->multispecies_calorically_perfect_physics->density_ref*this->multispecies_calorically_perfect_physics->u_ref_sqr);
    }
    if(istate==dim+2){
    ...
    Parameters::AllParameters const *const param)
{
    ...
    template class InitialConditionFunction_MultiSpecies_CaloricallyPerfectBase <PHILIP_DIM, PHILIP_SPECIES, PHILIP_DIM+PHILIP_SPECIES+1,double>;
