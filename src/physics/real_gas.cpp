                                                ...
                                                const std::array<real,nstate> &conservative_soln2) const
{
    ...
    const std::array<real,nspecies> Rs = compute_Rs(this->Ru);
    std::array<real,nspecies> Cv1 = compute_species_specific_Cv(temp1); // nondimensional mass value
    std::array<real,nspecies> Cv2 = compute_species_specific_Cv(temp2); // nondimensional mass value
    ...
    for(int idim=0; idim<dim; idim++){
        this->pcout << "vel1 : " << vel1[idim] << " vel2 : " << vel2[idim] << std::endl;
    ...
    for(int flux_dim=0; flux_dim<dim; flux_dim++){
        ...
        pressure_avg /= (this->gam_ref*this->mach_ref_sqr);
        // this->pcout << "The consistent pressure average:  " << pressure_avg << std::endl;
        conv_num_split_flux[0][flux_dim] = rho_flux;
        // Momentum equation
        for (int velocity_dim=0; velocity_dim<dim; ++velocity_dim){
            conv_num_split_flux[1+velocity_dim][flux_dim] = rho_flux*vel_avg[velocity_dim];
        }
        conv_num_split_flux[1+flux_dim][flux_dim] += pressure_avg; // Add diagonal of pressure

        // Energy equation
        conv_num_split_flux[dim+1][flux_dim] = energy_flux + (conv_num_split_flux[1+flux_dim][flux_dim] * vel_avg[flux_dim]);
