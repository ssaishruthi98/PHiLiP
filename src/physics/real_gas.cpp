...
{
    std::cout<<"Entropy variables for RealGas has not been implemented yet."<<std::endl;
    std::abort();
...
::compute_species_specific_Cp ( const real temperature ) const
{
    ...
    // std::cout << "dim temp:   " << dimensional_temperature  << " temp:  " << temperature << std::endl;
    int temperature_zone;
    // species loop
    real_gas_cap->GetNASACAP_TemperatureIndex(dimensional_temperature, real_gas_cap->Sp_TempIndex);
    // std::cout << "Outputing Sp_TempIndex: ";
    for(int s=0; s<nspecies; ++s) {
        // std::cout << real_gas_cap->Sp_TempIndex[s] << ", ";
    }
    real_gas_cap->NASACAP_GetHSCp(real_gas_cap->H_T, real_gas_cap->S_T, real_gas_cap->Cp_T, real_gas_cap->Cv_T, dimensional_temperature*this->Ru);
    for (int s=0; s<nspecies; ++s) 
    { 
    ...
    // std::cout << std::endl << "Liki's Cp: ";
    // for(int s=0; s<nspecies; ++s) {
    //     std::cout << Cp[s] << ", ";
    // }
    // std::cout << std::endl << "Julien's Cp: ";
    // for(int s=0; s<nspecies; ++s) {
    //     std::cout << real_gas_cap->Cp_T[s] <<", ";
    // }
...
::compute_species_specific_heat_ratio ( const std::array<real,nstate> &conservative_soln ) const
{
    ...
    for (int s=0; s<nspecies; ++s) 
    {
        std::cout << "liki Cp:  " << Cp[s] << "  julien Cp:  " << real_gas_cap->Cp_T[s] << std::endl;
        std::cout << "liki Cv:  " << Cv[s] << "  julien Cv:  " << real_gas_cap->Cv_T[s] << std::endl;
        gamma[s] = Cp[s]/Cv[s];
        std::cout << "liki gamma:  " << gamma[s] << "  julien gamma:  " << real_gas_cap->Cp_T[s]/real_gas_cap->Cv_T[s] << std::endl;
    }
...
::compute_sound ( const std::array<real,nstate> &conservative_soln ) const
{
    ...
    const real sound = compute_mixture_from_species(mass_fractions, species_sound);
    return sound;
}
    ...
    const dealii::Point<dim>                  &evaluation_points) const
{
    ...
    if constexpr (std::is_same<real,double>::value) {
        ...
        std::array<real,nspecies> species_sound = compute_species_speed_of_sound(conservative_soln);
        std::cout << "this line is just to prevent compile error:   " << species_sound[0] << std::endl;
