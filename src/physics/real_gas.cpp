...
// Modification: The function now works with a nondimensionalized version of the NASA9 polynomial
template <int dim, int nspecies, int nstate, typename real>
std::array<real,nspecies> RealGas<dim,nspecies,nstate,real>
::compute_species_specific_Cp ( const real temperature ) const
{
    ...
    { 
        // main computation
        if (dimensional_temperature < 0.8*NASACAPTemperatureLimits[ispecies][0] || dimensional_temperature > 1.2*NASACAPTemperatureLimits[ispecies][1]) {
            std::cout << "Species Cp Calculation WARNING: Temperature exceeds the " << species_name[ispecies] << " polynomial limits by more than 20%..." << std::endl;
        }
        Cp[ispecies] = 0;
...
// This function has been modified by Shruthi
// Modification: Uses a nondimensionalized version of the NASA9 polynomial
template <int dim, int nspecies, int nstate, typename real>
std::array<real,nspecies> RealGas<dim,nspecies,nstate,real>
::compute_species_specific_enthalpy ( const real temperature ) const
{
    ...
    { 
        // main computation
        if (dimensional_temperature < 0.8*NASACAPTemperatureLimits[ispecies][0] || dimensional_temperature > 1.2*NASACAPTemperatureLimits[ispecies][1]) {
            std::cout << "Species Enthalpy Calculation WARNING: Temperature exceeds the " << species_name[ispecies] << " polynomial limits by more than 20%..." << std::endl;
        }
        real h_ref = 0;
        for (int icoeffs = 0; icoeffs < 6; ++icoeffs)
...
::compute_species_specific_internal_energy( const real temperature ) const
{
    ...
    for (int s=0; s<nspecies; ++s) 
    {
        e[s] = (this->R_ref*this->temperature_ref/this->u_ref_sqr)*(h[s] -  Rs[s]*temperature);
    }
    ...
    const real temperature) const
{
    ...
    { 
        // main computation
        if (dimensional_temperature < 0.8*NASACAPTemperatureLimits[ispecies][0] || dimensional_temperature > 1.2*NASACAPTemperatureLimits[ispecies][1]) {
            std::cout << "Species Entropy Calculation WARNING: Temperature exceeds the " << species_name[ispecies] << " polynomial limits by more than 20%..." << std::endl;
        }
                                  ...
                                  const std::array<real,nstate> &conservative_soln2) const
{
    ...
    if(two_point_num_flux_type == two_point_num_flux_enum::KG) {
        ...
        // std::cout << "The Chandrashekar two-point flux is currently being implemented...Not completed..." << std::endl;
                                                 ...
                                                 const std::array<real,nstate> &conservative_soln2) const
{
    ...
    for (int ispecies = 0; ispecies < nspecies; ++ispecies) {
        ...
        pressure_diagonal += this->Rs[ispecies] * (compute_mean(rho_species1[ispecies],rho_species2[ispecies]));
    }
    // std::cout << std::endl;
    ...
    for (int flux_dim = 0; flux_dim < dim; ++flux_dim)
    {
        ...
        for (int ispecies = 0; ispecies < nspecies - 1; ++ispecies) {
            ...
            std::cout << " ch flux = " << conv_num_split_flux[index][flux_dim]  << " kg flux = " << conv_num_split_flux_kg[index][flux_dim];
            std::cout << std::endl;

            // Energy equation
            conv_num_split_flux[dim+1][flux_dim] += (energy_flux_species_sum[ispecies]-0.5*vel_sqr_avg[flux_dim]) * conv_num_split_flux[index][flux_dim];
        }
        // Add last species contribution to energy flux
        conv_num_split_flux[dim+1][flux_dim] += (energy_flux_species_sum[nspecies-1]-0.5*vel_sqr_avg[flux_dim])* (log_mean_species_densities[nspecies-1] * vel_avg[flux_dim]);
