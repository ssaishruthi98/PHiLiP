        ...
        const int boundary_type) const
{
    ...
    if((on_boundary && (navier_stokes_physics->thermal_boundary_condition_type == thermal_boundary_condition_enum::adiabatic))
        ...
        // Step 1,2: Primitive solution and Gradient of primitive solution
        const std::array<dealii::Tensor<1,dim,real>,nstate> primitive_soln_gradient = this->navier_stokes_physics->convert_conservative_gradient_to_primitive_gradient(solution, solution_gradient);
        const std::array<real,nstate> primitive_soln = this->navier_stokes_physics->convert_conservative_to_primitive(solution); // from Euler
