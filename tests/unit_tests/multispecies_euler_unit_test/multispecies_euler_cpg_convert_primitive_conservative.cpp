...
int main (int argc, char * argv[])
{
    ...
    for (auto cell : grid.active_cell_iterators()) {
        for (unsigned int v=0; v < dealii::GeometryInfo<dim>::vertices_per_cell; ++v) {
            ...
            primitive_soln = multispecies_calorically_perfect_euler_physics.convert_conservative_to_primitive(conservative_soln);
            conservative_soln2 = multispecies_calorically_perfect_euler_physics.convert_primitive_to_conservative(primitive_soln);
