...
int main (int argc, char * argv[])
{
    ...
    PHiLiP::Physics::MultiSpecies_ThermallyPerfect_Euler<dim, nspecies, nstate, double> multispecies_thermally_perfect_euler_physics = 
            PHiLiP::Physics::MultiSpecies_ThermallyPerfect_Euler<dim, nspecies, nstate, double>(&all_parameters, all_parameters.euler_param.gamma_gas, all_parameters.euler_param.mach_inf);
    ...
    for (auto cell : grid.active_cell_iterators()) {
        for (unsigned int v=0; v < dealii::GeometryInfo<dim>::vertices_per_cell; ++v) {
            ...
            for (int s=0; s<nstate; s++) {
                conservative_soln[s] = multispecies_thermally_perfect_euler_physics.manufactured_solution_function->value(vertex, s);
            }
            entropy_variables = multispecies_thermally_perfect_euler_physics.compute_entropy_variables(conservative_soln);
            conservative_soln2 = multispecies_thermally_perfect_euler_physics.compute_conservative_variables_from_entropy_variables(entropy_variables);
            ...
            if(multispecies_thermally_perfect_euler_physics.compute_mixture_pressure(conservative_soln) < TOLERANCE) std::abort();
            if(multispecies_thermally_perfect_euler_physics.compute_sound(conservative_soln) < TOLERANCE) std::abort();
